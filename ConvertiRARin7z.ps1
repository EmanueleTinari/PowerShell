...
# (ITA) Pone nella variabile $DirectoryDiLavoro il percorso completo dello script, eliminando il nome dello stesso compreso di esstensione.
...
# (ITA) Pone nella variabile $LogFile il contenuto della variabile $DirectoryDiLavoro, concatena uno slash, concatena il contenuto della variabile
...
foreach ($CurrentRar in $Rar)
  {
    ...
    # (ITA) Estrae con un RegEx dalla variabile $StringTest solo numero di file e cartelle contenute nell'archivio RAR corrente e crea coi due
    #       dati un array $Matches.
